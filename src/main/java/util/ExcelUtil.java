...
public class ExcelUtil {
    ...
    public static String EXCEL_TEMPLATE_FORMAT_WRONG = "模板格式错误，请检查";
    ...
    public static Workbook getWorkbook(MultipartFile file) throws Exception {
        ...
        if (excel2003L.equals(suffix)) {
            ...
            throw new Exception("EXCEL的文件格式有,只支持.xls和.xlsx");
        }
        return wb;
    }
     ...
     * 检验模板标题是否正确（只能验证一级标题）
     */
    public static void validateHeader(Sheet sheet, String[] header) throws Exception {
        //检验模板格式
        ...
        for (int i = 0; i < header.length; i++) {
            ...
            if (cell == null) {
                throw new Exception("模板格式错误，请检查");
            }
            String v = cell.getStringCellValue().replace("*", "");
            if (!v.trim().equals(header[i])) {
                throw new Exception(EXCEL_TEMPLATE_FORMAT_WRONG);
            }
     ...
     * 自动调整列宽(中文支持)
     */
    public static void autoSizeAllColumn2(Sheet sheet) {
        List<Integer> rows = new ArrayList<>();
        for (int i = 0; i <= sheet.getLastRowNum(); i++) {
            rows.add(sheet.getRow(i).getPhysicalNumberOfCells());
        }
